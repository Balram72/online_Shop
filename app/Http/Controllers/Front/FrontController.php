...
class FrontController extends Controller
{
    ...
    public function add_to_cart(Request $request){
            ...
            $result= DB::table('products_attr')
                ...
                $products_attr_id=$result[0]->id;

                $getAvaliableQty = getAvaliableQty($product_id,$products_attr_id);
                ...
                $check=DB::table('cart')
                    ...
                    ->where(['product_attr_id' => $products_attr_id])
                    ->get();
                ...
                if(isset($check[0])){
                    ...
                    $id=DB::table('cart')->insertGetId([
                        ...
                        'product_attr_id' => $products_attr_id,
                        'qty' =>$pqty,
    ...
    public function login_process(Request $request){
        ...
        if(isset($result[0])){
            ...
            if($status==0){
                return response()->json(['status'=>"error",'msg'=>"Your Account Has been Decativated"]);
            }
    ...
    public function forgot_password_change_process(Request $request){
            ...
            ->update([
                ...
                return response()->json(['status'=>'success','msg'=>'Paaword Changed']);
                ...
                foreach($getAddToCartTotalItem as $list){
                    ...
                    if($request->payment_type=='Gateway'){
                        ...
                        curl_setopt($ch, CURLOPT_HTTPHEADER,
                            array("X-Api-Key:test_e6b1cac863e050b4f7359a4b74d",
                                "X-Auth-Token:test_f9a39ccc4cfcf663097c915a26a"));
                        $payload = Array(
                            'purpose' => 'Buy Product',
                            'amount' =>$final_amt,
                            'phone' => $request->mobile,
                            'buyer_name' =>$request->name,
                            'redirect_url' => 'http://127.0.0.1:8000/payment_redirect',
                            'send_email' => true,
                            'send_sms' => true,
                            'email' => $request->email,
    ...
    public function payment_redirect(Request $request){
        if($request->get('payment_status')!==null && $request->get('payment_id')!==null && $request->get('payment_request_id')!==null){
    ...
    public function myaccount(Request $request){
        if($request->session()->has('FRONT_USER_LOGIN')) {
            $uid = $request->session()->get('FRONT_USER_ID');
            $result['customerdata'] = DB::table('customers')
                ->where('id', $uid)
                ->first(); 
        }
    ...
    {
        return view('front/contact'); // Create this view to display the form
    }

    public function manageSend(Request $request)
    {
        ...
        if ($query) {
            ...
            Mail::send('front/emailcontact', $data, function ($message) use ($data) {
                $message->to('balrampanda87@gmail.com')
