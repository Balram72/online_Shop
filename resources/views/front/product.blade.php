        ...
        <div class="aa-product-details-area">
                    ...
                    <div class="simpleLens-thumbnails-container">
                      ...
                      @if(isset($product_images[$product[0]->id][0]))
                       @foreach($product_images[$product[0]->id] as $list)
                              <a data-big-image="{{asset('storage/media/productsMultipalImage/'.$list->images)}}" data-lens-image="{{asset('storage/media/productsMultipalImage/'.$list->images)}}" class="simpleLens-thumbnail-wrapper" href="#">
                                <img src="{{asset('storage/media/productsMultipalImage/'.$list->images)}}" width="50px">
                              </a>     
                          @endforeach
                      @endif
                    </div>
                  ...
                  @if($product_attr[$product[0]->id][0]->color_id>0)
                    ...
                    <div class="aa-color-tag">
                      @foreach ($product_attr[$product[0]->id] as $attr)
                        ...
                        onclick="change_product_color_image('{{asset('storage/media/productAttrImage/'.$attr->attr_image)}}',
                        '{{ $attr->color }}')"></a>
                        @endif
                      @endforeach
                    </div>
                ...
                <div class="aa-product-review-area">
                  ...
                  <!-- review form -->
                     <div class="form-group">
                       ...
                       <textarea class="form-control" rows="1" id="message" name="review" required></textarea>
                     </div>
          ...
          <div class="aa-product-related-item">
            ...
            <ul class="aa-product-catg aa-related-item-slider">
              ...
              @foreach($related_product as $relatedArr)
              <li>
                <figure>
                  <a class="aa-product-img" href="{{ url('product/'.$relatedArr->slug)}}">
                  <img src="{{asset('storage/media/'.$relatedArr->image)}}" alt="{{ $relatedArr->name }}"></a>
                  <a class="aa-add-card-btn"href="{{ url('product/'.$relatedArr->slug)}}"><span class="fa fa-shopping-cart"></span>Add To Cart</a>
                  <figcaption>
                    <h4 class="aa-product-title"><a href="{{ url('product/'.$relatedArr->slug)}}">{{ $relatedArr->name }}</a></h4>
                    <span class="aa-product-price">Rs.{{ $related_product_attr[$relatedArr->id][0]->price }}</span><span class="aa-product-price"><del>Rs.{{ $related_product_attr[$relatedArr->id][0]->mrp }}</del></span>
                  </figcaption>
                </figure>                          
              </li>
              @endforeach                          
